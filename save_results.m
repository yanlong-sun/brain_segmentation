...
for num_pred= 4 : length(pred_file)
    ...
    target_size = size(imread([test_data_tif_path, case_name, '/', case_name,'_',  num2str(10001), '.tif' ]));
    crop_window = centerCropWindow2d([256, 256], target_size);
    first_slice_path = ['./data/test/',case_name, '/', case_name,'_', num2str(10001), '.tif' ];
    first_slice = imread(first_slice_path);
    slices = imcrop(first_slice, crop_window);
    for i = 2 : (length(slice_fileNames)-2)/2
        ...
        single_slice = imcrop(single_slice, crop_window);
        figure(4)
        imshow(single_slice)
    ...
    for j = 1 : n1
        ...
        if j == 1
            ...
            imshow(preds)
            preds = imcrop(preds, crop_window);
        end
        single_pred = mat2gray(pred_img(:, :, j));
        single_pred = imcrop(single_pred, crop_window);
        imshow(single_pred)
