...
test_data_nii_path = '../test_tr_data/slices/';
...
slices_nii_folder=dir(test_data_nii_path);
slices_nii_file={slices_nii_folder.name};


for num_nii = 15 : length(slices_nii_file)
    case_name = slices_nii_file(num_nii);
    case_name = char(case_name);
    case_name = case_name(1 : end-7)    

    v_slices = load_untouch_nii([test_data_nii_path, case_name, '.nii.gz']);  
    v_masks = load_untouch_nii([test_data_nii_path, case_name, '_ss.nii.gz']);
